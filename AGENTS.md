...
| **Predicción de demanda + Benchmark anónimo** | Service `ocupacion-franjas.ts` (ocupación normalizada al pico histórico por día, on-demand), GET `/api/reportes/ocupacion` + tab "Ocupación" en reportes (toggle Reales/Demo, 4 StatCards, heatmap rediseñado con 8 niveles de color y leyenda); `ops-console/lib/benchmark.ts` (cross-tenant, buckets anti-identificación ≥5, snapshots del último run), GET/POST `/api/internal/benchmark` + GET `/api/reportes/benchmark` (campo `_opsError` para ops caído) + tab "Benchmark"; migración ops `0003_benchmark_snapshot`; WF-16 cron nocturno. Tests `benchmark.test.ts` 6/6 pass. Commits `f0f71cf`, `6debabc` | 03/08 |
