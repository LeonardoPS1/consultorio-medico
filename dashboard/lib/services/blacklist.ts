...
export const blacklistService = {
  async list(options: ListBlacklistOptions = {}) {
    ...
    if (search) condList.push(or(like(blacklist.motivo, `%${search}%`)));
  ...
  async getById(id: string) {
    ...
    if (row.entry.creadoPor) {
      const [medico] = await db
        ...
        .from(medicos)
        .where(eq(medicos.id, row.entry.creadoPor))
        .limit(1);
  ...
  async getStats() {
    ...
    const activos = await db
      ...
      .from(blacklist)
      .where(baseCond)
      .groupBy(blacklist.activo);

    return {
      total: Number(total?.total || 0),
      activos: activos.reduce(
        (acc, a) => ({ ...acc, [a.esActivo ? 'activos' : 'inactivos']: Number(a.total) }),
    ...
    const [row] = await db
      ...
      .where(
        and(
          ...
          or(sql`${blacklist.bloqueadoHasta} IS NULL`, sql`${blacklist.bloqueadoHasta} > NOW()`),
        ),
      )
      .limit(1);
