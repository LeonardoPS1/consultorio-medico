...
export const derivacionesService = {
  async list(options: ListDerivacionesOptions = {}) {
    ...
    if (medicoId) condList.push(or(eq(derivaciones.medicoOrigenId, medicoId), eq(derivaciones.medicoDestinoId, medicoId)));
    ...
    if (destinoIds.length > 0) {
      const destinoRows = await db.select({
        id: medicos.id,
        nombre: medicos.nombre,
      }).from(medicos).where(and(sql`${medicos.id} = ANY(${destinoIds})`, sql`${medicos.deletedAt} IS NULL`));
    ...
    if (origenIds.length > 0) {
      const origenRows = await db.select({
        id: medicos.id,
        nombre: medicos.nombre,
      }).from(medicos).where(and(sql`${medicos.id} = ANY(${origenIds})`, sql`${medicos.deletedAt} IS NULL`));
  ...
  async getById(id: string) {
    ...
    const medicosRows = await db.select({
      id: medicos.id,
      nombre: medicos.nombre,
      especialidad: medicos.especialidad,
    }).from(medicos).where(and(sql`${medicos.id} = ANY(${medicosIds})`, sql`${medicos.deletedAt} IS NULL`));

    const medicosMap = new Map(medicosRows.map(m => [m.id, { nombre: m.nombre, especialidad: m.especialidad }]));
    ...
    return {
      ...
      medicoDestinoNombre: row.derivacion.medicoDestinoId ? (medicosMap.get(row.derivacion.medicoDestinoId)?.nombre || '—') : null,
      medicoDestinoEspecialidad: row.derivacion.medicoDestinoId ? (medicosMap.get(row.derivacion.medicoDestinoId)?.especialidad || null) : null,
    };
  },

  async create(input: CreateDerivacion) {
    const [nueva] = await db.insert(derivaciones).values({
    ...
    if (input.medicoDestinoId) {
      try {
        ...
        const [medicoDestino] = await db.select({ usuarioId: medicos.usuarioId })
          .from(medicos).where(eq(medicos.id, input.medicoDestinoId)).limit(1);
      ...
      } catch { /* No bloquear si notificación falla */ }
    }
  ...
  async update(id: string, input: UpdateDerivacion) {
    ...
    if (input.estado !== undefined) {
      ...
      if (input.estado === 'aceptada' || input.estado === 'rechazada' || input.estado === 'completada') {
    ...
    if (input.estado) {
      try {
        const { notificacionesService } = await import('@/lib/services/notificaciones');
        const [medicoOrigen] = await db.select({ usuarioId: medicos.usuarioId })
          .from(medicos).where(eq(medicos.id, existing.medicoOrigenId)).limit(1);
  ...
  async getStats(medicoId?: string) {
    const cond = medicoId
      ? and(sql`${derivaciones.deletedAt} IS NULL`, or(eq(derivaciones.medicoOrigenId, medicoId), eq(derivaciones.medicoDestinoId, medicoId)))
    ...
    const estados = await db.select({
      estado: derivaciones.estado,
      total: count(),
    }).from(derivaciones).where(cond).groupBy(derivaciones.estado);

    const gravedades = await db.select({
      gravedad: derivaciones.gravedad,
      total: count(),
    }).from(derivaciones).where(cond).groupBy(derivaciones.gravedad);

    return {
      total: Number(total?.total || 0),
      porEstado: estados.reduce((acc, e) => ({ ...acc, [e.estado]: Number(e.total) }), {} as Record<string, number>),
      porGravedad: gravedades.reduce((acc, g) => ({ ...acc, [g.gravedad]: Number(g.total) }), {} as Record<string, number>),
    };
  },
  ...
  async getMedicos() {
    return db.select({
      ...
      nombre: medicos.nombre,
      especialidad: medicos.especialidad,
    }).from(medicos).where(and(sql`${medicos.deletedAt} IS NULL`, eq(medicos.activo, true)))
