...
export const consentimientosService = {
  ...
  async getById(id: string) {
    ...
    if (row.consentimiento.medicoId) {
      const [medico] = await db.select({ nombre: medicos.nombre })
        .from(medicos).where(eq(medicos.id, row.consentimiento.medicoId)).limit(1);
  ...
  async create(input: CreateConsentimiento) {
    const [nuevo] = await db.insert(consentimientos).values({
  ...
  async update(id: string, input: UpdateConsentimiento) {
    ...
    if (input.fechaFirma !== undefined) updateData.fechaFirma = input.fechaFirma ? new Date(input.fechaFirma) : null;
  ...
  async eliminar(id: string) {
    ...
    await db.update(consentimientos).set({ deletedAt: new Date() }).where(eq(consentimientos.id, id));
    return { success: true };
  },
  ...
  async getStats() {
    ...
    const porTipo = await db.select({
      tipo: consentimientos.tipo,
      total: count(),
    }).from(consentimientos).where(baseCond).groupBy(consentimientos.tipo);

    return {
      total: Number(total?.total || 0),
      porTipo: porTipo.reduce((acc, t) => ({ ...acc, [t.tipo]: Number(t.total) }), {} as Record<string, number>),
    };
  },
};
