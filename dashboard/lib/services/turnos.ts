...
export const turnosService = {
    ...
    return cache.getOrSet(
      ...
      async () => {
        ...
        const statsRows = await db
          .select({ estado: turnos.estado, total: count() })
          .from(turnos)
          .where(statsWhere)
        ...
        for (const r of statsRows) {
          statsPorEstado[r.estado] = Number(r.total);
          statsTotal += Number(r.total);
        }
  ...
  async create(input: CreateTurno) {
    try {
      ...
      // Validar médico
      const [med] = await db
        .select({ id: medicos.id, horarios: medicos.horarios, nombre: medicos.nombre })
        .from(medicos)
      ...
      const horariosMedico = (med.horarios || {}) as Record<
        string,
        {
          activo?: boolean;
          inicio?: string;
      ...
      if (tieneHorariosConfigurados) {
        ...
        if (horarioDia.tipo === 'partido' && horarioDia.inicio2 && horarioDia.fin2) {
          ...
          const enManiana =
            horarioDia.inicio &&
            horarioDia.fin &&
            enRango(horaTurno, horarioDia.inicio, horarioDia.fin);
          const enTarde = enRango(horaTurno, horarioDia.inicio2, horarioDia.fin2);
          if (!enManiana && !enTarde) {
            conflict(
              `El medico ${med.nombre} atiende los ${diaNombre} de ${horarioDia.inicio} a ${horarioDia.fin} y de ${horarioDia.inicio2} a ${horarioDia.fin2}. El turno solicitado (${horaTurno}) esta fuera del horario.`,
            );
          }
        } else {
          ...
          if (horarioDia.inicio && horarioDia.fin) {
            if (horaTurno < horarioDia.inicio || horaTurno >= horarioDia.fin) {
              conflict(
                `El medico ${med.nombre} atiende los ${diaNombre} de ${horarioDia.inicio} a ${horarioDia.fin}. El turno solicitado (${horaTurno}) esta fuera del horario.`,
              );
            }
          }
        }
      ...
      const bloqueo = await db
        .select({ titulo: bloqueosAgenda.titulo, tipo: bloqueosAgenda.tipo })
        .from(bloqueosAgenda)
        .where(
          and(
            eq(bloqueosAgenda.medicoId, input.medicoId),
      ...
      if (bloqueo.length > 0) {
        const tipoLabel =
          bloqueo[0].tipo === 'vacaciones'
            ? 'de vacaciones'
            : bloqueo[0].tipo === 'feriado'
              ? 'feriado'
              : 'no disponible';
        conflict(
          `El medico esta ${tipoLabel}: "${bloqueo[0].titulo}". No se pueden agendar turnos en esta fecha.`,
        );
      }
      ...
      if (input.tipoConsulta === 'telemedicina') {
        telemedicinaService.configurarSala(nuevo.id, fechaHora).catch((err) => {
          safeError(
            '[LiveKit] Error configurando sala virtual:',
  ...
  async update(id: string, input: UpdateTurno & { skipWaitlist?: boolean }) {
    try {
      ...
      if (input.estado === 'en_atencion') {
        ...
        data.canceladoPor = 'dashboard';
        data.motivoCancelacion = input.motivoCancelacion || null;
      ...
      try {
        const hayCambiosRelevantes =
          input.fecha !== undefined || input.hora !== undefined || input.estado !== undefined;
        ...
        if (hayCambiosRelevantes) {
          const [paciente] = await db
            ...
            .where(
              and(
                eq(pacientes.id, turnoActualizado.pacienteId),
          ...
          const [medico] = await db
            ...
            .from(medicos)
            .where(
              and(eq(medicos.id, turnoActualizado.medicoId), sql`${medicos.deletedAt} IS NULL`),
            )
            .limit(1);

          const fechaHoraStr =
            turnoActualizado.fechaHora instanceof Date
          ...
          const gcalAction =
            turnoActualizado.estado === 'cancelada'
              ? ('delete' as const)
      ...
      if (input.estado === 'cancelada' && !input.skipWaitlist) {
        waitlistService
          .buscarCandidato(turnoActualizado.medicoId, turnoActualizado.sucursalId || undefined)
          ...
          .catch((err) => {
            safeError(
              '[Waitlist] Error al procesar cancelación:',
              err instanceof Error ? { message: err.message } : err,
            );
          });
      }
  ...
  async delete(id: string) {
    ...
    try {
      ...
      const fechaHoraStr =
        existe.fechaHora instanceof Date
