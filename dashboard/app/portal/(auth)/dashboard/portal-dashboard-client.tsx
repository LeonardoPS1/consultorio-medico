...
function getEstadoRecetaColor(estado: string) {
  switch (estado) {
    case 'activa': return 'bg-emerald-100 text-emerald-800 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-800';
    case 'vencida': return 'bg-red-100 text-red-800 border-red-200 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800';
...
function getSistemaSaludBadge(s: string | null) {
  if (!s) return null;
  const colors: Record<string, string> = {
    fonasa: 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800',
    isapre: 'bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-800',
    particular: 'bg-green-100 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800',
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch('/api/portal/encuestas/responder', {
        ...
        body: JSON.stringify({ turnoId: turnoActual.id, puntaje, comentario: comentario || undefined }),
      });
      if (!res.ok) {
      ...
      <Card className="shadow-sm border-blue-100 dark:border-blue-900/50 bg-gradient-to-br from-blue-50/50 to-white dark:from-blue-950/20 dark:to-gray-900">
        <CardContent className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <h3 className="font-semibold text-sm text-gray-900 dark:text-gray-100">Calificá tu atención</h3>
          ...
          {turnosSinEncuesta.length > 1 && (
            <div className="flex gap-1.5 mb-3 flex-wrap">
              {turnosSinEncuesta.slice(0, 3).map((t) => (
                <button
                  key={t.id}
                  onClick={() => { setSelectedTurno(t.id); setPuntaje(0); setComentario(''); setSubmitted(false); }}
              ...
              {turnosSinEncuesta.length > 3 && (
                <span className="text-[11px] text-gray-400 self-center">+{turnosSinEncuesta.length - 3} más</span>
              )}
            </div>
          )}
          ...
          <div className="flex items-center gap-1.5 mb-3">
            ...
            {puntaje > 0 && (
              <span className="ml-1 text-sm font-medium text-gray-600 dark:text-gray-400">
                {puntaje === 1 ? 'Muy malo' : puntaje === 2 ? 'Malo' : puntaje === 3 ? 'Regular' : puntaje === 4 ? 'Bueno' : 'Excelente'}
              </span>
            )}
          </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Turnos', valor: turnos.length, icon: Calendar, gradient: 'from-blue-50 to-blue-100/50 dark:from-blue-950/30 dark:to-blue-900/20', iconColor: 'text-blue-600 dark:text-blue-400', textColor: 'text-blue-700 dark:text-blue-300' },
          { label: 'Próximos', valor: turnosProximos.length, icon: Clock, gradient: 'from-emerald-50 to-emerald-100/50 dark:from-emerald-950/30 dark:to-emerald-900/20', iconColor: 'text-emerald-600 dark:text-emerald-400', textColor: 'text-emerald-700 dark:text-emerald-300' },
          { label: 'Recetas', valor: recetas.length, icon: HeartPulse, gradient: 'from-purple-50 to-purple-100/50 dark:from-purple-950/30 dark:to-purple-900/20', iconColor: 'text-purple-600 dark:text-purple-400', textColor: 'text-purple-700 dark:text-purple-300' },
        ].map((stat, i) => (
      ...
      <div className={`${staggerItem}`} style={staggerDelay(3)}>
        <Card className="shadow-sm border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 transition-colors duration-300">
          <CardContent className="p-4">
            <h2 className="font-semibold text-sm text-gray-500 dark:text-gray-400 mb-3 uppercase tracking-wider">Mis datos</h2>
            <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
              <div>
                <p className="text-xs text-gray-400 dark:text-gray-500">Nombre completo</p>
                <p className="font-medium text-gray-900 dark:text-gray-100">{paciente.nombre} {paciente.apellido}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 dark:text-gray-500">Teléfono</p>
                <p className="font-medium text-gray-900 dark:text-gray-100">{formatCLPhone(paciente.telefono)}</p>
              </div>
              {paciente.rut && (
                <div>
              ...
              {paciente.email && (
                <div>
                  ...
                  <p className="font-medium text-gray-900 dark:text-gray-100 truncate">{paciente.email}</p>
                </div>
              )}
            ...
            {turnosProximos.length === 0 ? (
              <Card className="bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800">
                <CardContent className="flex flex-col items-center justify-center py-10 text-center">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 font-medium">No tienes turnos próximos</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Los turnos aparecerán aquí cuando los agendes</p>
                </CardContent>
              </Card>
            ) : (
                  ...
                  <div key={t.id} className={`animate-fade-in-up`} style={{ animationDelay: `${i * 0.08}s`, animationFillMode: 'both' }}>
                    <Card className="border-l-4 overflow-hidden shadow-sm border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5" style={{ borderLeftColor: getTurnoColor(t.estado) }}>
                      <CardContent className="p-4">
                        ...
                        {t.motivo && (
                          <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">{t.motivo}</p>
                        )}
                        {t.medicoNombre && (
            ...
            {turnosPasados.length === 0 && historial.length === 0 ? (
              <Card className="bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800">
                <CardContent className="flex flex-col items-center justify-center py-10 text-center">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Tus visitas anteriores aparecerán aquí</p>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-3">
            ...
            {recetas.length === 0 ? (
              <Card className="bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800">
                <CardContent className="flex flex-col items-center justify-center py-10 text-center">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 font-medium">Sin recetas activas</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Las recetas aparecerán aquí cuando el médico las recete</p>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-3">
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="font-semibold text-gray-900 dark:text-gray-100">{r.medicamento}</p>
                          </div>
                        ...
                        {r.duracion && (
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Duración: {r.duracion}</p>
                        )}
                        {r.indicaciones && (
