...
export default function PortalAuthLayout({ children }: { children: React.ReactNode }) {
  return (
    ...
    >
      <header
        className="sticky top-0 z-10 bg-background/80 dark:bg-background/80 backdrop-blur-md border-b border-border/60 shadow-sm"
        style={{
          transition:
            'background 300ms var(--ease-out), border-color 300ms var(--ease-out), box-shadow 300ms var(--ease-out)',
        }}
      >
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          <Link href="/portal/dashboard" className="font-semibold text-lg tracking-tight">
