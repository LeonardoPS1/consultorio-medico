...
const DEFAULT_STATS_RESPONSE = {
  ...
  proximosTurnos: [] as Array<{ hora: string; paciente: string; tipo: string; estado: string; medico: string }>,
    ...
    const data = await cache.getOrSet(cacheKey, async () => {
      try {
        ...
        const [turnosHoy, turnosAyer] = await Promise.all([
          db.select({ total: count() }).from(turnos)
        ...
        const [convActivas, alertas, mensajesStats] = await Promise.all([
          db.select({ total: count() }).from(conversaciones)
          ...
          db.select({ total: count() }).from(pacienteEventos)
          ...
          isMedico
            ? db.select({
              ...
              .where(and(gte(mensajes.createdAt, thirtyDaysAgo), sql`EXISTS (SELECT 1 FROM conversaciones c WHERE c.id = ${mensajes.conversacionId} AND c.medico_id = ${sessionMedicoId})`))
              .limit(1)
            : db.select({
                pacienteHoy: sql<number>`COUNT(*) FILTER (WHERE ${mensajes.rol} = 'paciente' AND ${mensajes.createdAt} >= ${todayStart})`,
                paciente30d: sql<number>`COUNT(*) FILTER (WHERE ${mensajes.rol} = 'paciente' AND ${mensajes.createdAt} >= ${thirtyDaysAgo})`,
        ...
        try {
          const [eventos, ultimosMensajes] = await Promise.all([
            db.select({ tipo: pacienteEventos.tipo, descripcion: pacienteEventos.descripcion, createdAt: pacienteEventos.createdAt })
              .from(pacienteEventos).orderBy(desc(pacienteEventos.createdAt)).limit(10),
            db.select({
              contenido: mensajes.contenido, createdAt: mensajes.createdAt,
              ...
              .orderBy(desc(mensajes.createdAt)).limit(5),
          ]);

          for (const ev of eventos) {
            if (ev.descripcion) {
              actividadReciente.push({
                hora: String(ev.createdAt?.toLocaleTimeString?.('es-CL', { hour: '2-digit', minute: '2-digit' }) ?? ''),
          ...
          for (const m of ultimosMensajes) {
            ...
            actividadReciente.push({
              hora: String(m.createdAt?.toLocaleTimeString?.('es-CL', { hour: '2-digit', minute: '2-digit' }) ?? ''),
        ...
        return {
          kpis: [
            ...
            { title: 'Mensajes Hoy', value: String(mensajesHoyCount), change: '0', type: 'today' },
          ],
          proximosTurnos,
          actividadReciente,
          sistema: {
