...
// ─── Sub-componente KPI Card (memoizado) ──────────────────
  ...
  return (
    ...
    >
      <Card className="relative overflow-hidden transition-[transform,box-shadow] duration-200 hoverable:hover:-translate-y-[1px] hoverable:hover:shadow-card-hover">
        <div
          className={`absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r ${up ? 'from-emerald-400 to-emerald-600' : 'from-red-400 to-red-600'}`}
        />
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="text-sm font-medium text-muted-foreground">{titulo}</CardTitle>
          <div
            className={`p-1.5 rounded-md ${up ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400' : 'bg-red-50 text-red-600 dark:bg-red-950/50 dark:text-red-400'}`}
          >
    ...
    >
      <Card className="relative overflow-hidden">
        ...
        <CardContent>
          ...
          <p className={`text-xs flex items-center gap-1 mt-1 ${color || 'text-muted-foreground'}`}>
            {subtitle}
          </p>
        </CardContent>
      </Card>
...
export default function ReportesPage() {
  ...
  const exportarReporte = useCallback(
    (formato: 'pdf' | 'excel') => {
      if (!data) return;
      const periodoLabel =
        periodo === 'semana' ? 'Semanal' : periodo === 'mes' ? 'Mensual' : 'Anual';
        ...
        import('@/lib/export-reporte-excel')
          .then(({ exportReporteExcel }) => {
            ...
            toast({
              ...
              description: `Reporte ${periodoLabel} exportado correctamente`,
            });
          })
          .catch(() => {
            toast({
      ...
      if (ventana) {
        ...
        toast({
          title: '❌ Error',
          description: 'Permití ventanas emergentes',
          variant: 'destructive',
        });
      }
      toast({
        title: '📊 Reporte generado',
        description: `Período ${periodoLabel} - Abrí la ventana para guardar como PDF`,
      });
    },
    [data, periodo],
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center rounded-lg border p-1">
            ...
            <Button
              variant={periodo === 'año' ? 'secondary' : 'ghost'}
              size="sm"
          ...
          <Badge
            variant="outline"
            className={`group relative text-[10px] cursor-default select-none ${data._demo ? 'text-amber-600 border-amber-200 dark:text-amber-400 dark:border-amber-800' : 'text-emerald-600 border-emerald-200 dark:text-emerald-400 dark:border-emerald-800'}`}
          >
      ...
      <Tabs defaultValue="general">
        ...
        <TabsContent value="turnos" className="mt-4 space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard
              title="Total Turnos"
              value={String(data.turnosKpis.total)}
              subtitle={`${data.turnosKpis.cambioTotal} este ${periodo === 'año' ? 'año' : periodo === 'mes' ? 'mes' : 'período'}`}
            ...
            <StatCard
              ...
              value={data.turnosKpis.asistencia}
            ...
            <StatCard
              ...
              value={data.turnosKpis.duracion}
        ...
        <TabsContent value="pacientes" className="mt-4 space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard
              ...
              value={String(data.pacientesKpis.total)}
            ...
            <StatCard
              title="Nuevos"
              value={String(data.pacientesKpis.nuevos)}
            ...
            <StatCard
              title="Edad Promedio"
              value={`${data.pacientesKpis.edadPromedio} años`}
          ...
          <Card>
            ...
            <CardContent>
              <NuevosPacientesChart
                data={data.nuevosPacientesLabels.map((label, i) => ({
                  label,
                  valor: data.nuevosPacientes[i] || 0,
                }))}
              />
            </CardContent>
          </Card>
        ...
        <TabsContent value="whatsapp" className="mt-4 space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Volumen WhatsApp</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-2xl font-bold text-primary">{data.calidadRespuesta.tasa}</p>
                  <p className="text-xs text-muted-foreground">Tasa Respuesta</p>
                </div>
                <div>
                  <p className="text-2xl font-bold text-primary">{data.calidadRespuesta.tiempo}</p>
                  <p className="text-xs text-muted-foreground">Tiempo Medio</p>
                </div>
                <div>
                  <p className="text-2xl font-bold text-primary">
                    {data.calidadRespuesta.msgsPorConv}
                  </p>
                  <p className="text-xs text-muted-foreground">Msgs/Conv</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="comparativa" className="mt-4 space-y-4">
          <Card>
            ...
            <CardContent>
              <ComparativaMensual
                data={(data._comparativa || {}) as ComparativaData}
                periodo={periodo}
              />
            </CardContent>
          </Card>
          {isAdvancedReports && data.prediccion && data.prediccion.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Comparativa vs Predicción</CardTitle>
              </CardHeader>
              <CardContent>
                <PrediccionDemanda data={data.prediccion || []} />
              </CardContent>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <p className="text-xl font-bold">{data.ejecutivo.totalIngresos}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Ocupación</p>
                    <p className="text-xl font-bold">{data.ejecutivo.tasaOcupacion}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Satisfacción</p>
                    <p className="text-xl font-bold">{data.ejecutivo.satisfaccion}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">NPS</p>
                    <p className="text-xl font-bold">{data.ejecutivo.nps}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
...
// ─── Tab General (extraído para legibilidad) ──────────────
                  ...
                  <div key={item.intencion}>
                    <div className="flex justify-between text-sm mb-1.5">
                      <span className="flex items-center gap-2 font-medium">
                        <div
                          className={`h-6 w-6 rounded-md bg-gradient-to-br ${coloresIntencion[idx]} flex items-center justify-center`}
                        >
