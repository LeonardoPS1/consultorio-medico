...
import {
  ...
  Loader2, AlertCircle, Award, DollarSign, CalendarX, AlertTriangle,
} from 'lucide-react';
...
const NuevosPacientesChart = dynamic(
  () => import('@/components/charts/nuevos-pacientes-chart'),
  { ssr: false, loading: () => <div className="h-48 animate-pulse rounded-lg bg-muted" /> },
);
const VolumenWhatsAppChart = dynamic(
  () => import('@/components/charts/volumen-whatsapp-chart'),
  { ssr: false, loading: () => <div className="h-48 animate-pulse rounded-lg bg-muted" /> },
);
...
const ComparativaMensual = dynamic(
  () => import('@/components/reportes/comparativa-mensual'),
  { ssr: false, loading: () => <div className="h-48 animate-pulse rounded-lg bg-muted" /> },
);
const PrediccionDemanda = dynamic(
  () => import('@/components/reportes/prediccion-demanda'),
  { ssr: false, loading: () => <div className="h-48 animate-pulse rounded-lg bg-muted" /> },
);
const ConversionFunnel = dynamic(
  () => import('@/components/reportes/conversion-funnel'),
  { ssr: false, loading: () => <div className="h-48 animate-pulse rounded-lg bg-muted" /> },
);
...
const iconosIntencion = [
  MessageSquare, Calendar, AlertTriangle, Activity, TrendingUp, MessageSquare,
];
      ...
      <Tabs defaultValue="general">
        ...
        <TabsContent value="pacientes" className="mt-4 space-y-4">
          ...
          <Card>
            ...
            <CardContent><NuevosPacientesChart data={data.nuevosPacientesLabels.map((label, i) => ({ label, valor: data.nuevosPacientes[i] || 0 }))} /></CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="comparativa" className="mt-4 space-y-4">
          <Card>
            ...
            <CardContent><ComparativaMensual data={(data._comparativa || {}) as ComparativaData} periodo={periodo} /></CardContent>
          </Card>
        ...
        {data.ejecutivo && (
          <TabsContent value="ejecutivo" className="mt-4 space-y-4">
            <Card>
              <CardHeader><CardTitle className="text-lg flex items-center gap-2"><DollarSign className="h-5 w-5 text-primary" />Resumen Ejecutivo</CardTitle></CardHeader>
              <CardContent>
...
function TabGeneral({ data }: { data: ReporteApiResponse }) {
  return (
    <TabsContent value="general" className="mt-4 space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card><CardHeader><CardTitle className="text-lg flex items-center gap-2"><BarChart3 className="h-5 w-5 text-primary" />Turnos por día</CardTitle></CardHeader><CardContent className="pb-3"><TurnosChart data={data.turnos} /></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-lg flex items-center gap-2"><PieChart className="h-5 w-5 text-primary" />Intenciones de mensajes</CardTitle></CardHeader><CardContent>
              ...
              return (
                <div key={item.intencion}>
                  ...
                  <div className="h-3 bg-muted rounded-full overflow-hidden">
                    <div className={`h-full rounded-full bg-gradient-to-r ${coloresIntencion[idx]} origin-left transition-transform duration-300 ease-out`} style={{ transform: `scaleX(${item.porcentaje / 100})`, width: '100%' }} />
                  </div>
                </div>
              );
            })}
